...
static int m46_chk_cmdstr_max(struct m46_cli_cmd_tbl *);
static int m46_print_all_cand(struct m46_cli_cmd_tbl *);
static void m46_chk_tab(char *, int *, int *);
static void m46_print_buf(int, char **, char *, int *, int *);
static void m46_cli_clr_line(char *, int *, int *);
static void m46_set_term(struct termios *);
static void m46_restore_term(struct termios *);
static void m46_print_sup(int, char **, int, char *, struct m46_cli_cmd_tbl *, int *, int *);
static void m46_print_cand(int, char **, int, char *, struct m46_cli_cmd_tbl *, int, int *, int *);
static void init_hist(hist_t *);

int main (int argc, char **argv) {

	struct termios save_term;
	hist_t hist[M46E_CLI_HISTORY_MAX];
	char buf[M46E_CLI_BUFSIZE], save_buf[M46E_CLI_BUFSIZE], ch;
	hist_t *cur, *save_cur;
	int hist_flg;
	...
	for (;;) {
		...
		for (;;) {
			...
			if (isprint(ch)) {
				...
				/* TAB */
				m46_chk_tab(buf, &end_pos, &cursor_pos);
					...
					if (ch == 'A') {
						...
						if (strlen(cur->prev->str) != 0) {
							...
							if (cur->prev != save_cur) {
								m46_cli_clr_line(buf, &end_pos, &cursor_pos);
		...
		if (strlen(buf)) {
			...
			/* history保存 */
			memcpy(save_cur->str, buf, M46E_CLI_BUFSIZE-1);
			cur = save_cur->next;

			if (strcmp(buf, "exit") == 0) {
				m46_restore_term(&save_term);
				exit(0);
			} else {
				m46_call_cmd(buf);
			}
...
void m46_call_cmd(char *buf)
{

	struct m46_cli_cmd_tbl *cmdp;
	int argc, i, ret, chk_ret = 0;
	int command_found = 0;
	char *argv[M46E_TOKEN_MAX], *p, *save_p;
	char token[M46E_TOKEN_MAX][M46E_TOKEN_LEN_MAX];
	char *cmd_str_p = NULL;
	char *cmd_exp_p = NULL;
...
CALL_NEXT:
	...
	if (cmdp->call_func != NULL) {
		...
		} else if (chk_ret == M46E_CHKERR_IPV4ADDR) {
			printf("%s : invalid ipv4 address.\n", cmd_str_p);
		} else if (chk_ret == M46E_CHKERR_IPV4MASK_VALUE) {
			printf("%s : invalid mask value.\n", cmd_str_p);
		} else if (chk_ret == M46E_CHKERR_IPV6ADDR) {
			printf("%s : invalid ipv6 address.\n", cmd_str_p);
		} else if (chk_ret == M46E_CHKERR_INVALID_VALUE) {
			printf("%s : invalid value. expected %s.\n", cmd_exp_p, cmd_str_p);
		} else if (chk_ret == M46E_CHKERR_FILE_NOT_FOUND) {
			printf("file not found.\n");
		} else if (chk_ret == M46E_CHKERR_NSNAME_LEN) {
			printf("NameSpace name length must be 65 characters or less.\n");
		} else if (chk_ret == M46E_CHKERR_NSNAME) {
			printf("invalid NameSpace name.\n");
		} else if (chk_ret == M46E_CHKERR_IFNAME_LEN) {
			printf("device name length must be 16 characters or less.\n");
		} else if (chk_ret == M46E_CHKERR_IF_EXSIST) {
			printf("specified device name already exists.\n");
		} else if (chk_ret == M46E_CHKERR_IP_CMD_ERROR) {
			printf("ip command error.\n");
		} else if (chk_ret == M46E_CHKERR_IF_NOT_EXSIST) {
			printf("specified device does not exist.\n");
		} else if (chk_ret == M46E_CHKERR_SWITCH) {
			printf("invalid flag : expected <on-off>\n");
		} else {			//その他エラーはすべてM46E_CHKERR_SYNTAX
			for (i = 0; i < argc; i++) {
				printf("%s ", argv[i]);
			}
...
static void m46_chk_tab(char *buf, int *end_pos_p, int *cursor_pos_p)
{
	struct m46_cli_cmd_tbl *cmdp, *cur_cmdp;
	int argc, cand, cnt, i, j;
	char tab_str[M46E_CLI_BUFSIZE], token[M46E_TOKEN_MAX][M46E_TOKEN_LEN_MAX];
	char t_buf[M46E_CLI_BUFSIZE];
	char *argv[M46E_TOKEN_MAX], *save_p, *tab_str_p, *p;
	...
	if (argc == 0) {
		/* root全表示 */
		if (m46_print_all_cand(cmdp) == 1) {
			/* 候補が１つだったら補完 */
			m46_cli_clr_line(buf, end_pos_p, cursor_pos_p);
...
NEXT:
				...
				if (cmdp->next == NULL) {
					...
					m46_print_buf(argc, argv, buf, end_pos_p, cursor_pos_p);
					break;
				} else {
					/* 次のツリーへ */
	...
	if (cnt == 1) {
		/* 補完処理 */
		m46_print_sup(argc, argv, i, buf, cur_cmdp, end_pos_p, cursor_pos_p);
	} else if (cnt > 1) {
		/* 候補表示 */
		m46_print_cand(argc, argv, i, buf, cur_cmdp, cnt, end_pos_p, cursor_pos_p);
	} else {
		/* ヒットなし */
		;
	}
...
static int m46_chk_cmdstr_max(struct m46_cli_cmd_tbl *cmdp)
{
	int max_len;
...
static int m46_print_all_cand(struct m46_cli_cmd_tbl *cmdp)
{
	int max_len, cnt;
	char tab_str[M46E_CLI_BUFSIZE];
	char tab_fmt[M46E_TAB_SIZE];
	char *p;
	...
	/* 出力フォーマット作成(最大文字+3) */
	max_len = (m46_chk_cmdstr_max(cmdp) + 3);
	...
	for (p = tab_str; cmdp->cmd_str != NULL; cmdp++, cnt++) {
		...
		if (strlen(tab_str) > M46E_TAB_WIDTH) {
			printf("\n%s", tab_str);
			memset(tab_str, 0, sizeof(tab_str));
			p = tab_str;
		} else {
 ...
 * 入力バッファの再表示
 */
static void m46_print_buf(int argc, char **argv, char *buf, int *end_pos_p, int *cursor_pos_p)
{
	int i;
 ...
 * 現在入力されたコマンドラインのクリア
 */
static void m46_cli_clr_line(char *buf, int *end_pos_p, int *cursor_pos_p)
{
	/* 表示、入力バッファをクリア */
...
static void m46_set_term(struct termios *save_term)
{
...
static void m46_restore_term(struct termios *save_term)
{
 ...
 * 補完処理
 */
static void m46_print_sup(int argc, char **argv, int pos, char *buf, struct m46_cli_cmd_tbl *cmdp, int *end_pos_p, int *cursor_pos_p)
{
	...
	if (argc > 1) {
		m46_print_buf((argc - 1), argv, buf, end_pos_p, cursor_pos_p);
	} else {
		m46_cli_clr_line(buf, end_pos_p, cursor_pos_p);
	}
 ...
 * 一部の候補を表示
 */
static void m46_print_cand(int argc, char **argv, int pos, char *buf, struct m46_cli_cmd_tbl *cmdp, int cnt, int *end_pos_p, int *cursor_pos_p)
{
	int max_len, i;
	char tab_str[M46E_CLI_BUFSIZE], tab_fmt[M46E_TAB_SIZE];
	char *p;
	char tmp_str[cnt][M46E_CLI_BUFSIZE];
...
static void init_hist(hist_t *hist)
{
	...
	for (i = 0; i < M46E_CLI_HISTORY_MAX; i++) {
		hist[i].next = &hist[i+1];
		hist[i].prev = &hist[i-1];
	}

	hist[M46E_CLI_HISTORY_MAX-1].next = &hist[0];
	hist[0].prev = &hist[M46E_CLI_HISTORY_MAX-1];
